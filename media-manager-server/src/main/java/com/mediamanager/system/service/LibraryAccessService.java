...
public class LibraryAccessService {
    ...
    private boolean isAuthEnabled() {
        ...
        return sysConfigService.isEffectiveAuthEnabled(yamlAuthEnabled);
    ...
    private Optional<LibraryAccess> findAccessRow(SysUser user, Integer libraryId) {

        return libraryAccessRepository.findByUser_Id(user.getId()).stream()

                .filter(la -> libraryId.equals(la.getLibraryId()))
    ...
    public void assertCanScanLibrary(Integer libraryId) {
        if (!isAuthEnabled()) {
            return;
        }
        Optional<SysUser> user = securityCurrentUser.getCurrentUser();
        ...
        assertCanViewLibrary(libraryId);
        if (bypassesLibraryRestrictions(user.get())) {
            return;
        }
        if (hasGlobalAuthority("library:scan")) {
            return;
        }
        LibraryAccess access = findAccessRow(user.get(), libraryId)
                .orElseThrow(() -> new BusinessException(ErrorCode.LIBRARY_ACCESS_DENIED));
    ...
    public void assertCanDeleteLibrary(Integer libraryId) {
        if (!isAuthEnabled()) {
            return;
        }
        Optional<SysUser> user = securityCurrentUser.getCurrentUser();
        ...
        if (hasGlobalAuthority("library:delete")) {
            return;
        }
        LibraryAccess access = findAccessRow(user.get(), libraryId)
                .orElseThrow(() -> new BusinessException(ErrorCode.LIBRARY_ACCESS_DENIED));
    ...
    public void assertCanEditLibrary(Integer libraryId) {
    ...
    public void assertCanDeleteInLibrary(Integer libraryId) {
        ...
        if (!Boolean.TRUE.equals(access.getCanDeleteFile())) {
    ...
    public void assertCanEditItem(MediaItem item) {
        ...
        assertCanEditLibrary(item.getLibrary().getId());

    }



    public void assertCanDeleteItem(MediaItem item) {
        ...
        assertCanDeleteInLibrary(item.getLibrary().getId());
    ...
    /** Recycle bin entries are soft-deleted files on hidden items; only library scope applies. */

    public void assertCanViewRecycleFile(MediaFile file) {
    ...
    public void assertCanPurgeRecycleFile(MediaFile file) {

        assertCanViewRecycleFile(file);

        if (file.getMediaItem() != null && file.getMediaItem().getLibrary() != null) {

            assertCanDeleteInLibrary(file.getMediaItem().getLibrary().getId());
